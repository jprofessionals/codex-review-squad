...
const proposed = {
  ...
  namespace: "a collision-safe unique marketplace name, plugin name, and 0.3.3+codex.rg07-* cachebuster are generated for every run",
...
const cachebuster = `0.3.3+codex.rg07-${nonce}`;
...
try {
  assert.equal(JSON.parse(repoManifestBefore).version, "0.3.3", "repository manifest is not exactly 0.3.3");
  ...
  try {
    ...
    const repositoryUnchanged = repoManifestAfter.equals(repoManifestBefore) && JSON.parse(repoManifestAfter).version === "0.3.3";
