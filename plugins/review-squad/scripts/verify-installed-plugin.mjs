...
const proposed = {
  ...
  namespace: "a collision-safe unique marketplace name, plugin name, and 0.3.2+codex.rg07-* cachebuster are generated for every run",
...
const cachebuster = `0.3.2+codex.rg07-${nonce}`;
...
try {
  assert.equal(JSON.parse(repoManifestBefore).version, "0.3.2", "repository manifest is not exactly 0.3.2");
  ...
  try {
    ...
    const repositoryUnchanged = repoManifestAfter.equals(repoManifestBefore) && JSON.parse(repoManifestAfter).version === "0.3.2";
