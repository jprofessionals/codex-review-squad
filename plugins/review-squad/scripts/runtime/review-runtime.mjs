...
// plugins/review-squad/scripts/review-runtime.mjs
import fs5 from "node:fs";
import path5 from "node:path";
import process3 from "node:process";
import { fileURLToPath as fileURLToPath3 } from "node:url";

// plugins/review-squad/scripts/lib/detection.mjs
var import_yaml = __toESM(require_dist(), 1);
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
var here = path.dirname(fileURLToPath(import.meta.url));
var catalog = JSON.parse(fs.readFileSync(path.resolve(here, "..", "..", "references", "review-catalog.json"), "utf8"));
var installations = [
...
function scopeActivatesBmad(context) {
  return Boolean(
    context.requested || context.story || context.acceptance_criteria || context.lifecycle_command || context.bmad_config_change || context.generated_bmad_artifact
  );
}
function detectBmad(signals = {}, context = {}) {
  const diagnostics = [];
  const confirmed = [];
...
function bmadExtensionFromDetection(detection) {
...
// plugins/review-squad/scripts/migrate-report.mjs
import fs3 from "node:fs";
import path3 from "node:path";
import process from "node:process";

// plugins/review-squad/scripts/lib/report-validation.mjs
...
var referencesRoot = path2.resolve(here2, "..", "..", "references");
var readJson = (relative) => JSON.parse(fs2.readFileSync(path2.join(referencesRoot, relative), "utf8"));
var reportSchema = readJson("review-report.schema.json");
var bmadSchema = readJson("extensions/bmad/review-report-bmad.v1.schema.json");
var legacySchema = readJson("schemas/review-report.v1.1.schema.json");
function buildAjv(options = {}) {
  const ajv = new import__.default({ allErrors: true, strict: true, strictRequired: false, allowUnionTypes: true, ...options });
  (0, import_ajv_formats.default)(ajv);
  return ajv;
}
var reportValidator = buildAjv().compile(reportSchema);
var bmadValidator = buildAjv().compile(bmadSchema);
var legacyValidator = buildAjv({ strict: false }).compile(legacySchema);
...
function migrateReport(input) {
  if (input?.schema_version === "2.0") {
    const validation2 = validateReport(input);
    if (!validation2.valid) throw new Error(formatDiagnostics(validation2.diagnostics));
    return structuredClone(input);
  }
  const legacy = validateLegacyReport(input);
  if (!legacy.valid) throw new Error(formatDiagnostics(legacy.diagnostics));
  const bmadDecisions = /* @__PURE__ */ new Map();
  ...
  const report = {
    ...
    generator: { ...input.generator, version: "0.3.2" },
...
if (process.argv[1] && path3.basename(process.argv[1]) === "migrate-report.mjs" && new URL(import.meta.url).pathname === process.argv[1]) {
  ...
  try {
    const migrated = migrateReport(JSON.parse(fs3.readFileSync(args[0], "utf8")));
    const output = `${JSON.stringify(migrated, null, 2)}
`;
    if (args[2]) fs3.writeFileSync(args[2], output);
...
// plugins/review-squad/scripts/render-report.mjs
import fs4 from "node:fs";
import path4 from "node:path";
import process2 from "node:process";
var label = (value) => value.replaceAll("-", " ").replace(/\b\w/g, (letter) => letter.toUpperCase());
...
if (process2.argv[1] && path4.basename(process2.argv[1]) === "render-report.mjs" && new URL(import.meta.url).pathname === process2.argv[1]) {
  ...
  try {
    const markdown = renderReport(JSON.parse(fs4.readFileSync(args[0], "utf8")));
    if (args[2]) fs4.writeFileSync(args[2], markdown);
    else process2.stdout.write(markdown);
  } catch (error) {
...
function readJson2(file, label2) {
  try {
    return JSON.parse(fs5.readFileSync(file, "utf8"));
  } catch (error) {
    console.error(`${label2} ${file}: ${error.message}`);
    process3.exit(2);
  }
}
if (process3.argv[1] && fileURLToPath3(import.meta.url) === path5.resolve(process3.argv[1])) {
  const [command, ...args] = process3.argv.slice(2);
  if (command === "--help") {
    console.log(usage3());
    process3.exit(0);
  }
...
validate: [--legacy] <report.json>`);
    ...
    const report = readJson2(file, "REPORT_READ_ERROR");
...
${command}: <input.json> [--output <output-file>]`);
    ...
    try {
      const report = readJson2(args[0], "REPORT_READ_ERROR");
      const value = command === "render" ? renderReport(report) : `${JSON.stringify(migrateReport(report), null, 2)}
`;
      if (outputIndex === 1) fs5.writeFileSync(args[2], value);
      else process3.stdout.write(value);
    } catch (error) {
...
detect: <files.json>`);
    ...
    const files = readJson2(args[0], "DETECTION_INPUT_ERROR");
    ...
    process3.stdout.write(`${JSON.stringify(detectProjects(files), null, 2)}
`);
  } else {
    console.error(usage3());
    process3.exit(2);
